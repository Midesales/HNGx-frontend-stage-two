    ...
    return (
        <div className='px-32 py-10'>
            <div className='flex justify-between gap-1 items-center'>
                ...
                <button onClick={toggleMovies} className='text-[#BE123C] font-semibold  lg:text-lg flex items-center gap-1 lg:gap-3'>
                    See more <span className='lg:text-xl'>&gt;</span>
                </button>
            </div>
            <div className = 'grid md:grid-cols-3 lg:grid-cols-5 gap-y-6'>
                {movies.slice(0, visibleMovies).map((movie) => {
                const { title, release_date, poster_path } = movie;
                return (
                    <div key={movie.id} data-testid='movie-card' className = 'grow text-middle pt-10' >
                        <a href={`/Moviedetails/{$id}`}>
                            <img src={API_images + poster_path} alt="movie-poster" data-testid='movie-poster' />
                        </a>
                        <h2 data-testid='movie-title' className = 'font-bold text-lg'>{title}</h2>
